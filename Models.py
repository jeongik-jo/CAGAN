...
class Discriminator(kr.layers.Layer):
    ...
    def build(self, __input_shape):
        if hp.is_mnist:
            ...
            for i in range(4):
                ftr_vec = Dense(units=hp.cnt_dim * 2, activation=activation)(ftr_vec)
        if hp.is_dis_batch_op:
            std_vec = tf.sqrt(tf.math.reduce_variance(ftr_vec, axis=0, keepdims=True) + 1e-8)
            bias = tf.Variable(tf.zeros([1, 512]), name=self.name + '_b')
            ftr_vec = activation((Dense(units=512, use_bias=False)(ftr_vec) + Dense(units=512, use_bias=False)(std_vec)) / tf.sqrt(2.0) + bias)
        else:
            ftr_vec = Dense(units=512, activation=activation)(ftr_vec)
        adv_val = tf.squeeze(Dense(units=1)(ftr_vec))
        ctg_vec = Dense(units=hp.ctg_dim)(ftr_vec)
        self.model = kr.Model(inp, [adv_val, ctg_vec])
